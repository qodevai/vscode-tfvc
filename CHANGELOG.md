...
### Changed
- SOAP plumbing extracted to `@qodevai/tfs-soap` (published on npm with SLSA provenance). The extension now imports `TfvcSoapClient`, `TfvcUploadClient`, `DiscussionSoapClient`, `HttpClient`, `TfvcError`, and `classifyHttpError` from the lib instead of carrying its own copies under `src/ado/` and `src/`. Behaviour is unchanged — same SOAP envelopes, same upload protocol, same error classification — but ~1500 LoC of generic TFS client code now lives outside the extension and is independently versioned, tested, and consumable by other tools. `HttpClient` is constructed once per activation with `{ strictSSL, proxyUrl }` and threaded into every client (`AdoRestClient`, `DiscussionSoapClient`, `TfvcSoapClient`, `TfvcUploadClient`) instead of relying on module-level `setStrictSSL()` / `setProxyUrl()` globals — cleaner ownership and unblocks future per-host configuration. Test compile uses `module: "nodenext"` to consume the ESM-only lib via Node 22+'s `require(esm)` interop; runtime build is unaffected (esbuild still bundles to CJS for VS Code).
